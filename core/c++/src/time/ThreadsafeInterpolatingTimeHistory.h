...
class ThreadsafeInterpolatingTimeHistory {
	    	...
	    	for ( int i = 0; i < num_valid_samples; i++ ) {
	    		...
	    		if ( delta < 0 ) {
		    		if ( delta > nearest_preceding_timestamp_delta ) {
		    			 ...
		    			 */
		    			if ( p_nearest_following_obj != NULL ) break;
		    		}
	    		} else if ( delta > 0 ) {
    ...
    bool writeToDiskInDirectory(string directory)
    {
    	...
    	if (!dir.isDirectory() || !dir.canWrite()) {
    		printf("Directory parameter '" + dir + "' must be a writable directory.");
    		return false;
    	}
    	...
    	public:
    		bool accept(File* dir, const string& name) {
    			...
    			bool ends_with = (name.find(filename_suffix)==name.size()-filename_suffix.size()-1);
    	...
    	for ( File *p_matching_file : matching_files ) {
    		...
    		regex expression("[.][^.]+$");
    		string replacement("");
    		string file_name_prefix = regex_replace(file_name, expression, replacement);
    		string file_counter = file_name_prefix.substr(filename_prefix.length());
    ...
    bool writeToDiskInternal(PrintWriter& out) {
    	...
    	if ( num_valid_samples > 0 ) {
			...
			bool is_quantity_container = quantity.getContainedQuantityNames(quantity_names);
    		/* Write Header */
    		...
    		out.println(header);
     		...
     		for ( int i = 0; i < num_to_write; i++) {
    			...
    			value_string.insert_after(value_string.end(),string(entry_to_write.getTimestamp()));
        		...
        		if (is_quantity_container) {
        			...
        			for (IQuantity* contained_quantity : contained_quantities) {
        				...
        				for ( auto string_part : printable_string) {
        					value_string.insert_after(value_string.end(),string_part);
        				}
        			}
        		} else {
